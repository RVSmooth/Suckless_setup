	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				// XMoveResizeWindow(dpy, m->barwin, m->wx + sp, m->by + vp, m->ww -  2 * sp, bh); #code01
        resizebarwin(m);
			}
			focus(NULL);
			arrange(NULL);
...
// +		drw_text(drw, m->ww - tw - stw, 0, tw, bh, 0, stext, 0); #code01
	}
...
togglebar(const Arg *arg)
{
	...
	updatebarpos(selmon);
	// XMoveResizeWindow(dpy, selmon->barwin, selmon->wx + sp, selmon->by + vp, selmon->ww - 2 * sp, bh); #code01
	resizebarwin(selmon);
	if (showsystray) {
		XWindowChanges wc;
		if (!selmon->showbar)
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		if (m->barwin)
			continue;
		// m->barwin = XCreateWindow(dpy, root, m->wx + sp, m->by + vp, m->ww - 2 * sp, bh, 0, DefaultDepth(dpy, screen), #code01
		w = m->ww;
		if (showsystray && m == systraytomon(m))
			w -= getsystraywidth();
